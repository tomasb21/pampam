...
#' For \code{family="mgaussian"}, \code{y} is a matrix
...
#' (poisson) or \code{"mrelnet"} for the various types of
...
#' \item{beta}{For \code{"elnet"}, \code{"lognet"} and \code{"fishnet"}
#'  models, a \code{nvars x length(lambda)} matrix of
...
#'
#' # Relaxed
#' fit1r = pampam(x, y, relax = TRUE)  # can be used with any model
#'
#' # multivariate gaussian
#' y = matrix(rnorm(100 * 3), 100, 3)
#' fit1m = pampam(x, y, family = "mgaussian")
#' plot(fit1m, type.coef = "2norm")
#'
#' # binomial
#' g2 = sample(c(0,1), 100, replace = TRUE)
#' fit2 = pampam(x, g2, family = "binomial")
#' fit2n = pampam(x, g2, family = binomial(link=cloglog))
#' fit2r = pampam(x,g2, family = "binomial", relax=TRUE)
#' fit2rp = pampam(x,g2, family = "binomial", relax=TRUE, path=TRUE)
#'
#' # multinomial
#' g4 = sample(1:4, 100, replace = TRUE)
#' fit3 = pampam(x, g4, family = "multinomial")
#' fit3a = pampam(x, g4, family = "multinomial", type.multinomial = "grouped")
#' # poisson
#' N = 500
#' p = 20
#' nzc = 5
...
### Need to do this first so defaults in call can be satisfied
    ...
    nc=dim(y)
    if(is.null(nc)){
      ## only build a factor/indicator matrix for binomial or multinomial;
      ## for all other families (gaussian, poisson, cox, mgaussian, or a
      ## family object), leave y as-is and default nc to 1
      if(is.character(family)) family=match.arg(family)
      if(is.character(family) && family %in% c("multinomial")){
          y=as.factor(y)
          ntab=table(y)
          minclass=min(ntab)
      ...
      ## y was already provided as a matrix (e.g. mgaussian, or a
      ## pre-built indicator matrix) — nc should be the number of
      ## columns, not the full dim() vector
      nc=nc[2]
    }
...
### See whether its a call to glmnet or to glmnet.path, based on family arg
    if(!is.character(family)){
      ...
      if(any(cl==0)){
        ...
        fdev=pampam.control()$fdev
        if(fdev!=0) {
          pampam.control(fdev=0)
          on.exit(pampam.control(fdev=fdev))
        }
      }
