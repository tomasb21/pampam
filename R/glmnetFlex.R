...
#' fit1 <- pampam:::glmnet.path(x, y, family = binomial(link = "probit"))
glmnet.path <- function(x, y, weights=NULL, lambda = NULL, nlambda = 100,
    ...
    ### check on limits
    control <- pampam.control()
    if (thresh >= control$epsnr)
        warning("thresh should be smaller than pampam.control()$epsnr",
                call. = FALSE)
...
elnet.fit <- function(x, y, weights, lambda, alpha = 1.0, intercept = TRUE,
    ...
    warm_fit <- wls_fit[c("almc", "r", "xv", "ju", "vp", "mp", "cl", "nx",
