              ...
              parallel, trace.it, glmnet.call, cv.call, ...)
{
  ...
  glmnet.object = pampam(x, y, weights = weights, offset = offset,
...
#  if (parallel && require(foreach)) {
    outlist = foreach(i = seq(nfolds), .packages = c("pampam")) %dopar%
    {
      which = foldid == i
#      if (is.matrix(y))
      if (length(dim(y))>1)
      ...
      pampam(x[!which, , drop = FALSE], y_sub, lambda = lambda,
      ...
      outlist[[i]] = pampam(x[!which, , drop = FALSE],
...
#    if(subclass=="glmnetfit") attr(predmat,"family")=glmnet.object$family
  ...
  grouped=cvstuff$grouped
    ...
    warning("Option grouped=FALSE enforced in cv.pampam, since < 3 observations per fold",
            call. = FALSE)
    grouped = FALSE
    }
