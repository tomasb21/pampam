...
#' 'TRUE'.
#' @param x Input matrix of predictors used to refit the relaxed model.
...
### Fit a coefficient for each non null model
    ...
    if (internal.parms$itrace) {
        pampam.control(itrace = 0) ## disable glmnet reporting
        cat("Relaxed Fits\n")
        pb  <- utils::txtProgressBar(min = 0, max = length(sundex), initial = 0, style = 3)
        on.exit(pampam.control(itrace = internal.parms$itrace))
