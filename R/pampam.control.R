...
#' factory default = 100
#' @param itrace If 1 then progress bar is displayed when running \code{pampam}
#' and \code{cv.pampam}. factory default = 0
...
#' @examples
#'
#' pampam.control(fdev = 0)  #continue along path even though not much changes
#' pampam.control()  # view current settings
#' pampam.control(factory = TRUE)  # reset all the parameters to their default
#'
#' @export pampam.control
pampam.control <-
            ...
            itrace = 0, epsnr = 1e-06, mxitnr = 25, factory = FALSE)
{
  inquiry=!nargs()
   if (factory)
    invisible(pampam.control(fdev = 1e-05, devmax = 0.999,
