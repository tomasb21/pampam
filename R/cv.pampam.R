...
#'
#' The function runs \code{pampam} \code{nfolds}+1 times; the first to get the
...
#' standard deviation over the folds is computed.  Note that \code{cv.pampam}
...
#' cross-validate \code{alpha} as well, they should call \code{cv.pampam} with
#' a pre-computed vector \code{foldid}, and then use this same fold vector in
#' separate calls to \code{cv.pampam} with different values of \code{alpha}.
#' Note also that the results of \code{cv.pampam} are random, since the folds
#' are selected at random. Users can reduce this randomness by running
#' \code{cv.pampam} many times, and averaging the error curves.
#'
...
#'
#' @param x \code{x} matrix as in \code{pampam}.
#' @param y response \code{y} as in \code{pampam}.
#' @param weights Observation weights; defaults to 1 per observation
#' @param offset Offset vector (matrix) as in \code{pampam}
#' @param lambda Optional user-supplied lambda sequence; default is
#' \code{NULL}, and \code{pampam} chooses its own sequence. Note that this is done
...
#' For binomial model and binary data, \code{type.measure="mse"} amounts to the "Brier" score.
...
#' and can be ignored by most users. This refers to computing \code{nfolds} separate 
#' statistics, and then using
...
#' not apply to \code{type.measure="auc"}). With \code{grouped=FALSE} the log
...
#' @return an object of class \code{"cv.pampam"} is returned, which is a list
...
#' @seealso \code{pampam} and \code{plot}, \code{predict}, and \code{coef}
#' methods for \code{"cv.pampam"} and \code{"cv.relaxed"} objects.
...
#' set.seed(1011)
#' cvob1a = cv.pampam(x, y, type.measure = "mae")
#' plot(cvob1a)
#' title("Gaussian Family", line = 2.5)
#' set.seed(1011)
#' par(mfrow = c(2, 2), mar = c(4.5, 4.5, 4, 1))
#' cvob2 = cv.pampam(x, ly, family = "binomial")
#' plot(cvob2)
#' title("Binomial Family", line = 2.5)
#' frame()
#' set.seed(1011)
#' cvob3 = cv.pampam(x, ly, family = "binomial", type.measure = "class")
#' plot(cvob3)
#' title("Binomial Family", line = 2.5)
#' \dontrun{
#' cvob1r = cv.pampam(x, y, relax = TRUE)
#' plot(cvob1r)
#' predict(cvob1r, newx = x[, 1:5])
#' set.seed(1011)
#' cvob3a = cv.pampam(x, ly, family = "binomial", type.measure = "auc")
#' plot(cvob3a)
#' title("Binomial Family", line = 2.5)
#' set.seed(1011)
#' mu = exp(fx/10)
#' y = rpois(n, mu)
#' cvob4 = cv.pampam(x, y, family = "poisson")
#' plot(cvob4)
...
#' g3 = pampam:::rmult(p3)
#' set.seed(10101)
#' cvfit = cv.pampam(x, g3, family = "multinomial")
#' plot(cvfit)
...
#' y = rnorm(1e+05)
#' system.time(cv.pampam(x, y))
#' system.time(cv.pampam(x, y, parallel = TRUE))
#' }
#'
#' @export cv.pampam
cv.pampam <-
  function (x, y, weights=NULL, offset = NULL, lambda = NULL, type.measure = c("default","mse",
                                                           "deviance", "class", "auc", "mae"), nfolds = 10, foldid=NULL,  alignment=c("lambda","fraction"),grouped = TRUE, keep = FALSE, parallel = FALSE, gamma=c(0,.25,.5,.75,1),relax=FALSE,trace.it=0, ...)
{
    type.measure = match.arg(type.measure)
    alignment=match.arg(alignment)
  if (!is.null(lambda) && length(lambda) < 2)
      stop("Need more than one value of lambda for cv.pampam")
 ...
 if (is.null(weights))
  ...
  if (any(which))
    ...
    glmnet.call[[1]] = as.name("pampam")
    if(pampam.control()$itrace)trace.it=1
    else{
        if(trace.it){
            pampam.control(itrace=1)
            on.exit(pampam.control(itrace=0))
            }
        }
   if (is.null(foldid))
